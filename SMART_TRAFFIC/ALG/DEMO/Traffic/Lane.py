...
class Lane:
	...
	def AddVehicle(self,NumVehicles,mLaneNum,mTime,mLaneOrder,mTurn,mNewTurn,mTimeLimit,mMinGap):#
		...
		if len(self.Slots)>0:		
			self.mVh=Vehicle(mLaneNum,NumVehicles,mTime,self.MaxSpeeed,self.LengthControlZone,mTimeLimit,self.LengthInterSection,self.Slots[len(self.Slots)-1].GetLastCarPosition(mTime),self.Slots[len(self.Slots)-1].GetLastCarSpeed(),mMinGap)
		else:		
			self.mVh=Vehicle(mLaneNum,NumVehicles,mTime,self.MaxSpeeed,self.LengthControlZone,mTimeLimit,self.LengthInterSection,-1,-1,mMinGap)
		#NewTurn=False

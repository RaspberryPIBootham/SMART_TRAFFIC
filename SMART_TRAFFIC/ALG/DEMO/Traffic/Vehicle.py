...
class Vehicle:
	def __init__(self,mLaneNum,mID,mTimeEntered,mMaxSpeed,mLDist,mTimeLimit,mIDist,mPrevPos,mPrevSpeed,mMinGap):
		self.VID=mID
		...
		self.Path1="msgChannel01"
		self.Path2="msgChannel02"
		self.Path3="msgChannel03"
		self.Path4="msgChannel04"
		#self.Message=""
		...
		if mMaxSpeed==0 or mLDist/mMaxSpeed<0:
			...
			if self.Speed-mPrevSpeed>0 and mPrevPos-((self.Speed-mPrevSpeed)*(self.TimeToEnter-mTimeEntered))<mMinGap:
				...
				self.TimeToEnter=(mLDist/self.Speed)+mTimeEntered
				self.TimeToExit=((mLDist+mIDist)/self.Speed)+mTimeEntered
			self.Message=str(self.Speed)
			if mLaneNum==0:
				publish.single(self.Path1,self.Message,hostname=self.Server)
			elif mLaneNum==1:
				publish.single(self.Path2,self.Message,hostname=self.Server)
			elif mLaneNum==2:
				publish.single(self.Path3,self.Message,hostname=self.Server)
			if mLaneNum==3:
				publish.single(self.Path4,self.Message,hostname=self.Server)
